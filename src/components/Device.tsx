...
import { createDevice, deleteDevice, getDevice, updateDevice } from '../services/api';
...
const Device: React.FC = () => {
  ...
  const fetchData = async () => {
    getDevice().then(data => setItems(data));
    setData(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (ShowEdit) {
      ...
      await updateDevice(ids, payload);
    }
    else {
      payload = {
        name,
      ...
      <div>
        <div className='container my-5'>
          <div className='d-flex mb-3 justify-content-start'>
            <div className='h5 me-3'>Perangkat</div>
        ...
        <div className="container my-5">
          <form onSubmit={handleSubmit}>
            <div className='row'>
              ...
              <div className='col-6 mb-3'>
                <div className="form-group">
                  <label htmlFor="name" className="form-label">Kode</label>
                  <input type="text" id="name" className="form-control" value={kode} onChange={(e) => setKode(e.target.value)} required />
                </div>
              </div>
            </div>
