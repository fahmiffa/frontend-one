...
import { createGeng, deleteGeng, getgeng, updateGeng } from '../services/api';

const Gelanggang: React.FC = () => {
  ...
  const fetchData = async () => {
    getgeng().then(data => setItems(data));
    setData(true);
    setEdit(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (ShowEdit) {
      ...
      await updateGeng(ids, payload);
    }
    else {
      payload = {
        name
      };
      await createGeng(payload);
    }
    fetchData();
  };
    ...
    return (
      <div>
        <div className='container my-5'>
          <button className="btn btn-primary btn-sm mb-4" onClick={() => addData()}><i className='bi bi-plus'></i> Tambah</button>
          <h1 className='mb-4'>Gelanggang</h1>
